...
def setup_data(redis)
  ...
  puts "#{articles.size} articles loaded into the databases as hashes"

  # set up time zset (the times that the articles were posted), key is 'time:'
  articles.each_with_index do |article, i|
    redis.zadd("time:", article[:time], "article:#{i}")
  end
  puts "#{articles.size} time stamps loaded into the databases as a sorted set"
  ...
  puts "#{scores.size} scores loaded into the databases as a sorted set."
